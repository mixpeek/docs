...
| **OCR (per page)** | 5 credits | Medium (resolution, model) |
| **Video transcription (per min)** | 50 credits | Medium (model choice) |
...
- **Move inactive collections to cold storage**: Collections that haven't been queried in 30+ days can be evicted from Qdrant while remaining searchable via S3 Vectors (~100ms latency vs ~10ms).
- **Configure auto-evict rules**: Set `auto_evict` with a 30-day threshold on collections that see periodic traffic.
- **Archive stale collections**: Collections no longer needed can be archived to retain only metadata (no search, no storage cost).
- **Keep latency-sensitive collections active**: Production collections with strict SLA requirements should remain in Qdrant.

See [Storage Tiering](/operations/storage-tiering) for full documentation on lifecycle management.
...
- Reduce Qdrant costs with [Storage Tiering](/operations/storage-tiering)
