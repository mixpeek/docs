...
```mermaid
flowchart TD
  A[Electronics taxonomy - multimodal: image + video + text]
  A --> B[TV and Home Theater - doc: category; signals: image + video + text]
  B --> C[TVs - doc: product category; signals: image + video + text]
  C --> D[OLED TVs - doc: product subtype; signals: image + video]
  C --> E[LED TVs - doc: product subtype; signals: image + video]
  B --> F[Projectors - doc: product subtype; signals: image + video]
  A --> G[Audio - doc: category; signals: audio + text]
  G --> H[Headphones - doc: product subtype; signals: audio + text]
  G --> I[Speakers - doc: product subtype; signals: audio + text]
```

_Modality labels show the content used to match each node; multimodal means combining image, video, and text signals within one taxonomy for matching._
...
<Tabs>
  ...
  <Tab title="Hierarchical (Implicit)">
    - **Schema inferred**: Analyze collection schemas for overlapping fields or feature addresses to propose parent‑child edges
    - **Cluster inferred**: Use vector clustering across collections to detect nested groupings and containment
    - **LLM inferred**: Apply language models over names, metadata, and sample contents to suggest semantic hierarchies
    - **Hybrid**: Start implicit, then override specific nodes with explicit definitions
  </Tab>
