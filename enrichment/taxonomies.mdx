...
<Note>
  Taxonomies are reusable enrichment recipes. They join target documents with reference collections using a configured retriever, then copy selected fields into the results. Use flat taxonomies for 1:1 joins, or hierarchical taxonomies for multi‑level trees (explicit nodes and optional implicit inference).
</Note>
...
<Tabs>
  <Tab title="Flat">
    ...
    - Copy configured enrichment fields with replace/append semantics
  </Tab>
