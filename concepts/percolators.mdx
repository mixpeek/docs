...
Percolators in Mixpeek allow developers to search against saved queries, enabling efficient and dynamic content indexing. By leveraging the same syntax as normal hybrid multimodal searches, percolators save these queries for future use. When indexing an asset, you can specify to percolate the asset across your saved queries. For every match above a certain relevance threshold, results are appended to a matches collection.
