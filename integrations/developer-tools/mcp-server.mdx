...
Perfect for using with Claude Code or Claude Desktop:

```bash
# Run the MCP server
cd /path/to/mixpeek/server
...
```json
{
  "mcpServers": {
    "mixpeek": {
      "command": "python",
      "args": ["-m", "mixpeek_mcp.main", "--transport", "stdio"],
      "cwd": "/path/to/mixpeek/server",
      "env": {
...
```bash
# Start HTTP server
python -m mixpeek_mcp.main --transport http --host 0.0.0.0 --port 8080
```
...
# Run MCP server
CMD ["python", "-m", "mixpeek_mcp.main", "--transport", "http", "--host", "0.0.0.0", "--port", "8080"]
```
...
MCP_STREAM_TIMEOUT=600           # 10 minutes

# Rate limiting
MCP_RATE_LIMIT_PER_MINUTE=60

# Caching
MCP_ENABLE_RESPONSE_CACHE=false
MCP_CACHE_TTL=60

# Mixpeek Services (required)
MONGO_URI=mongodb://localhost:27017
REDIS_URL=redis://localhost:6379
QDRANT_URL=http://localhost:6334
...
├── tools/
│   ├── namespaces.py           # 5 namespace tools
│   ├── buckets.py              # 6 bucket tools
│   ├── collections.py          # 7 collection tools
│   ├── documents.py            # 5 document tools
│   ├── retrievers.py           # 7 retriever tools
│   ├── taxonomies.py           # 5 taxonomy tools
│   ├── clusters.py             # 4 cluster tools
│   ├── agents.py               # 3 agent tools
│   └── search.py               # 1 search tool
├── transports/
│   ├── stdio.py                # Stdio transport (local)
│   └── http_sse.py             # HTTP/SSE transport (deployed)
└── auth/
...
- **Direct Service Import**: Reuses existing Mixpeek services, shares database connections
- **Authentication**: Uses existing Mixpeek API key system
- **Streaming**: Supports SSE for real-time retriever and agent responses
- **Error Handling**: Converts Mixpeek errors to MCP-friendly JSON responses
