...
<CardGroup cols={2}>
  ...
  <Card title="Fully Managed" icon="cloud">
    Hosted service - no infrastructure to deploy or maintain
  </Card>
  <Card title="Simple Authentication" icon="key">
    Use your existing Mixpeek API key - no additional setup
  </Card>
  <Card title="Streaming Support" icon="stream">
    Real-time responses for retrievers and agents via SSE
  </Card>
</CardGroup>
...
**Solutions**:
- Verify the URL is `https://mcp.mixpeek.com`
- Check that your API key is correct and starts with `sk_`
- Ensure the Authorization header format is `Bearer YOUR_API_KEY`
- Restart Claude Desktop/Code after updating configuration

### Authentication Errors

**Issue**: Tools return "Unauthorized" or "Invalid API key"

**Solutions**:
- Verify your API key is valid at [mixpeek.com/dashboard](https://mixpeek.com/dashboard)
- Check that your API key has the required permissions
- Ensure you're not using an expired or revoked key
...
- Check that required parameters are provided and correctly formatted
- Verify resource IDs (namespace_id, collection_id, etc.) are valid
- Review tool parameter schemas in the documentation above

### Slow Response Times

**Issue**: MCP tools take a long time to respond

**Solutions**:
- Large file uploads may take time depending on file size
- Processing pipelines with multiple stages can be compute-intensive
- Check [status.mixpeek.com](https://status.mixpeek.com) for any service issues
- For large batch operations, consider using the REST API directly
...
- [Mixpeek API Documentation](/api-reference)
- [Get Your API Key](https://mixpeek.com/dashboard)

---

**Questions or issues?** Contact support at [support@mixpeek.com](mailto:support@mixpeek.com) or visit [mixpeek.com/support](https://mixpeek.com/support).
