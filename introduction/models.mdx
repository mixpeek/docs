...
<Steps>
  ...
  <Step title="Initiate Fine-Tuning">
    Use the Mixpeek API to start a fine-tuning job. Specify the
    model_id, the S3 bucket path, and any other relevant parameters:

    ```python
    mixpeek.models.tune(
      model_id="jinaai/jina-embeddings-v2-base-en",
      annotated_storage_connection="conn_123"
    )
    ```

    This will return a new model_id that you can use in your pipeline, for example: `model_1askdh2390`, which you'll then use in your pipeline.

  </Step>
  <Step title="Version Control">
    It's recommended to store the different versions of each fine-tune. If you append a
    `model_version` in the metadata of the pipeline it'll automatically be added to your database so you can do query-time filtering.
    
    This helps in tracking model evolutions and maintaining consistency across different versions.
