...
Collections start in the **active** lifecycle state with vectors in both Qdrant (fast search) and S3 Vectors (durable canonical store). You can transition collections to **cold** (S3 only) or **archived** (metadata only) to manage costs.

See [Storage Tiering](/operations/storage-tiering) for full details on lifecycle management.
...
6. **Cold-tier idle collections** to reduce Qdrant costs — see [Storage Tiering](/operations/storage-tiering).
