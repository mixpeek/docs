---
title: "Setup Pipelines via Github"
...
jobs:
  send-to-mixpeek:
    ...
    steps:
      ...
      - name: Prepare and Send Pipeline Data
        run: |
          files_changed=$(git diff --name-only ${{ github.event.before }} ${{ github.event.after }} | grep -E 'handler.py|config.json')
          for file in $files_changed; do
            if [[ $file == *handler.py ]]; then
              pipeline_dir=$(dirname $file)
              handler_content=$(cat ${{ github.workspace }}/$file)
              config_content=$(cat ${{ github.workspace }}/$pipeline_dir/config.json)
              alias=$(echo $config_content | jq -r '.alias')
              curl --location 'https://api.mixpeek.com/pipelines' \
                --header 'Authorization: Bearer ${{ secrets.MIXPEEK_API_KEY }}' \
                --header 'Content-Type: application/json' \
                --data "{
                  \"alias\": \"$alias\",
                  \"code\": \"$handler_content\",
                  \"destination\": $(echo $config_content | jq '.destination'),
                  \"source\": $(echo $config_content | jq '.source')
                }"
            fi
          done
```
...
- `destination`: This section specifies where the output of the pipeline will be sent or stored.
  ...
  If you don't want to integrate via GitHub workflows, you can do this:
