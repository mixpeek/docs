...
jobs:
  send-to-mixpeek:
    ...
    steps:
      ...
      - name: Send files to Mixpeek API
        run: |
          for file in $(git diff --name-only ${{ github.event.before }} ${{ github.event.after }} | grep -E 'handler.py|config.json'); do
            curl --location --request POST 'https://api.mixpeek.com/pipelines/build' \
              --header 'Authorization: Bearer ${{ secrets.MIXPEEK_API_KEY }}' \
              --form 'file=@${{ github.workspace }}/${file}' \
              --form 'filename=${file}'
          done
```
...
- `destination`: This section specifies where the output of the pipeline will be sent or stored.
  ...
  ```bash cURL
  curl --location 'https://api.mixpeek.com/pipelines/build' \
